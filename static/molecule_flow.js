 ...
 *  deterministic flows summing to 100
 ****************************************************************************/
"use strict";
...
function buildBigDAG(){
  ...
  // Root
  g_nodes.push({ id:"root", layer:0, x:50, y:400, terminal:false, reward:0, flowIn:0, flowOut:0, shapeParts:[] });
  ...
  for (let i = 1; i <= 9; i++){
    g_nodes.push({
      ...
      x:550,
      y:100 + i*75,
      terminal:true, reward:0, flowIn:0, flowOut:0, shapeParts:[]
    });
  }
  ...
  function addEdge(src, tgt){
    g_edges.push({
      ...
      flow:0, buildPart:null,
      length:0, x1:0,y1:0,x2:0,y2:0,angle:0
    });
  }
...
function renderDAG(){
  ...
  // terminal % labels
  const totalTerm = g_nodes.filter(n=>n.terminal).reduce((s,n)=>s+n.flowIn,0);
...
/*************************************************************************
 * PARTICLE SYSTEM (use moleculeParticles)
 *************************************************************************/
...
function updateMoleculeParticles(dtSec){
  for (let i = moleculeParticles.length-1; i>=0; i--){
    ...
    p.t += frac;
    if (p.t >= 1) moleculeParticles.splice(i,1);
  }
}
